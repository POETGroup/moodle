...
require_once(dirname(dirname(__FILE__)) . '/fieldlib.php');
...
function totara_hierarchy_cron() {
    ...
    require_once($CFG->dirroot.'/local/course_metadata/hierarchy/prefix/competency/cron.php');
    require_once($CFG->dirroot.'/local/course_metadata/hierarchy/prefix/goal/cron.php');
...
class hierarchy {
    ...
    */
    ...
    function display_add_item_button($page=0) {
        ...
        $url = new moodle_url('/local/course_metadata/hierarchy/item/edit.php', $options);
    ...
    function display_add_multiple_items_button($page=0) {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/local/course_metadata/hierarchy/item/bulkadd.php', $options);
    ...
    function display_bulk_actions_picker($can_add_item, $can_edit_item, $can_delete_item, $can_manage_type, $page=0) {
        ...
        $options = array();
        if ($can_add_item) {
            $options['/local/course_metadata/hierarchy/item/bulkadd.php?prefix='.$this->prefix.'&frameworkid='.$this->frameworkid.'&page='.$page] = get_string('add');
        }
        if ($can_delete_item) {
            $options['/local/course_metadata/hierarchy/item/bulkactions.php?action=delete&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('delete');
        }
        if ($can_edit_item) {
            $options['/local/course_metadata/hierarchy/item/bulkactions.php?action=move&prefix='.$this->prefix.'&frameworkid='.$this->frameworkid] = get_string('move');
        }
        if ($can_manage_type) {
            $options['/local/course_metadata/hierarchy/type/index.php?prefix='.$this->prefix.'#bulkreclassify'] = get_string('reclassifyitems' ,'totara_hierarchy');
        }
    ...
    function display_add_type_button($page=0) {
        global $OUTPUT;
        $options = array('prefix' => $this->prefix, 'frameworkid' => $this->frameworkid, 'page' => $page);
        $url = new moodle_url('/local/course_metadata/hierarchy/type/edit.php', $options);
    ...
    static function get_short_prefix($prefixname) {
        ...
        $libpath = $CFG->dirroot.'/local/course_metadata/hierarchy/prefix/'.$cleanprefixname.'/lib.php';
    ...
    static function load_hierarchy($prefix) {
        ...
        // Check file exists
        $libpath = $CFG->dirroot.'/local/course_metadata/hierarchy/prefix/'.$prefix.'/lib.php';
        if (!file_exists($libpath)) {
    ...
    function display_hierarchy_item($record, $include_custom_fields=false, $indicate_depth=true, $cfields=array(), $types=array()) {
        ...
        $out .= $OUTPUT->action_link(new moodle_url('/local/course_metadata/hierarchy/item/view.php', array('prefix' => $this->prefix, 'id' => $record->id)), format_string($record->fullname), null, array('class' => $cssclass));
    ...
    function display_hierarchy_item_custom_field_data($record, $cfields) {
        ...
        foreach ($cfields as $cf) {
            ...
            require_once($CFG->dirroot.'/local/course_metadata/field/'.$cf->datatype.'/field.class.php');
     ...
     *
     * require_once($CFG->dirroot.'/local/course_metadata/core/js/lib/setup.php');
    ...
    function export_select($baseurl=null) {
        global $CFG;
        require_once($CFG->dirroot.'/local/course_metadata/hierarchy/export_form.php');
        if (empty($baseurl)) {
    ...
    function parse_customfield($fieldid, $fieldvalue, $isexport=false) {
        ...
        $classfilepath = $CFG->dirroot . '/local/course_metadata/field/' . $datatype . '/field.class.php';
    ...
    public function get_permissions($context = null, $user = null) {
        ...
        if (file_exists($CFG->dirroot.'/local/course_metadata/hierarchy/prefix/'.$prefix.'/scale/lib.php')) {
