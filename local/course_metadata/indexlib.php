...
function customfield_edit_icons($field, $fieldcount, $typeid=0, $prefix, $can_edit, $can_delete) {
    ...
    if ($can_edit) {
        ...
        $editstr = $OUTPUT->action_icon(new moodle_url('/local/course_metadata/index.php', $params), new pix_icon('t/edit', $stredit), null, array('title' => $stredit));
    } else {
    ...
    if ($can_delete) {
        ...
        $deletestr = $OUTPUT->action_icon(new moodle_url('/local/course_metadata/index.php', $params), new pix_icon('t/delete', $strdelete), null, array('title' => $strdelete));
    } else {
    ...
    if ($field->sortorder > 1 && $can_edit) {
        ...
        $upstr = $OUTPUT->action_icon(new moodle_url('/local/course_metadata/index.php', $params), new pix_icon('t/up', $strmoveup), null, array('title' => $strmoveup));
    } else {
    ...
    if ($field->sortorder < $fieldcount && $can_edit) {
        ...
        $params['dir'] = 'down';
        $params['sesskey'] = sesskey();
        $downstr = $OUTPUT->action_icon(new moodle_url('/local/course_metadata/index.php', $params), new pix_icon('t/down', $strmovedown), null, array('title' => $strmovedown));
    } else {
...
function customfield_delete_field($id, $tableprefix) {
    ...
    $DB->delete_records('local_'.$tableprefix.'_metadata', array('fieldid' => $id));

    /// Try to remove the record from the database
    $DB->delete_records('local_'.$tableprefix.'_metadata_field', array('id' => $id));
...
function customfield_move_field($id, $move, $tableprefix, $prefix) {
    ...
    $field = $DB->get_record('local_'.$tableprefix.'_metadata_field', array('id' => $id), $subfields);

    /// Count the number of fields
    $fieldcount = $DB->count_records('local_'.$tableprefix.'_metadata_field');
    ...
    $swapfield = $DB->get_record('local_'.$tableprefix.'_metadata_field', $subfields);
    ...
    $DB->update_record('local_'.$tableprefix.'_metadata_field', $field);
    $DB->update_record('local_'.$tableprefix.'_metadata_field', $swapfield);
...
function customfield_list_datatypes() {
    ...
    if ($dirlist = get_directory_list($CFG->dirroot.'/local/course_metadata/field', '', false, true, false)) {
        foreach ($dirlist as $type) {
            $datatypes[$type] = get_string('customfieldtype'.$type, 'local_course_metadata');
...
function customfield_edit_field($id, $datatype, $typeid=0, $redirect, $tableprefix, $prefix, $navlinks=false) {
    ...
    if (!$field = $DB->get_record('local_'.$tableprefix.'_metadata_field', array('id' => $id))) {
        $field = new stdClass();
        $field->id = 0;
    ...
    require_once($CFG->dirroot.'/local/course_metadata/index_field_form.php');
    $field->descriptionformat = FORMAT_HTML;
    $field = file_prepare_standard_editor($field, 'description', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'local_course_metadata', 'textarea', $field->id);
    if ($field->datatype == 'textarea') {
        $field->defaultdataformat = FORMAT_HTML;
        $field = file_prepare_standard_editor($field, 'defaultdata', $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'local_course_metadata', 'textarea', $field->id);
    }
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if ($data = $fieldform->get_data()) {
            require_once($CFG->dirroot.'/local/course_metadata/field/'.$datatype.'/define.class.php');
        ...
        if (empty($id)) {
            $strheading = get_string('createnewfield', 'local_course_metadata', $datatypes[$datatype]);
        } else {
            $strheading = get_string('editfield', 'local_course_metadata', format_string($field->fullname));
        }
        ...
        if ($navlinks == false) {
            ...
            $PAGE->navbar->add(get_string($prefix.'plural', 'local_course_metadata'));
            $PAGE->navbar->add(get_string($prefix.'depthcustomfields', 'local_course_metadata'));
        }
...
function customfield_get_defined_fields($tableprefix,  array $where = array()) {
    global $DB;
    return $DB->get_records('local_'.$tableprefix.'_metadata_field', $where, 'sortorder ASC');
}
...
function customfield_reorder_fields($tableprefix) {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/local/course_metadata/core/utils.php');
    $rs = $DB->get_recordset('local_'.$tableprefix.'_metadata_field', array(), 'sortorder ASC');
    if ($types = totara_group_records($rs, 'typeid')) {
        foreach ($types as $unused => $fields) {
            ...
            foreach ($fields as $field) {
                ...
                $f->id = $field->id;
                $f->sortorder = $i++;
                $DB->update_record('local_'.$tableprefix.'_metadata_field', $f);
            }
        }
    }
    $rs->close();
