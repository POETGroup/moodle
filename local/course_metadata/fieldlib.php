...
/***** The following methods must be overwritten by child classes *****/
    ...
    function edit_field_add(&$mform) {
        print_error('error:abstractmethod', 'local_course_metadata');
    }
...
/***** The following methods may be overwritten by child classes *****/
    ...
    function edit_save_data($itemnew, $prefix, $tableprefix) {
        ...
        if ($dataid = $DB->get_field('local_'.$tableprefix.'_metadata', 'id', array($prefix.'id' => $itemnew->id, 'fieldid' => $data->fieldid))) {
            $data->id = $dataid;
            $DB->update_record('local_'.$tableprefix.'_metadata', $data);
        } else {
            $this->dataid = $DB->insert_record('local_'.$tableprefix.'_metadata', $data);
        }
        ...
        if ($this->is_unique()) {
            ...
            switch ($this->field->datatype) {
                ...
                case 'textarea':
                    ...
                    $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'local_course_metadata', $prefix, $itemnew->id);
            ...
            // search for a match
            if ($data != '' && $DB->record_exists_select('local_'.$tableprefix.'_metadata',
    ...
    function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule($this->inputname, get_string('customfieldrequired', 'local_course_metadata'), 'required', null, 'client');
        }
    }
...
/***** The following methods generally should not be overwritten by child classes *****/
    ...
    function load_data($itemid, $prefix, $tableprefix) {
        ...
        if (($this->fieldid == 0) || (!($field = $DB->get_record('local_'.$tableprefix.'_metadata_field', array('id' => $this->fieldid))))) {
            $this->field = NULL;
            $this->inputname = '';
        } else {
            $this->field = $field;
        ...
        if (!empty($this->field)) {
            if ($datarecord = $DB->get_record('local_'.$tableprefix.'_metadata', array($prefix.'id' => $this->itemid, 'fieldid' => $this->fieldid), 'id, data')) {
...
function customfield_load_data(&$item, $prefix, $tableprefix) {
    ...
    $fields = $DB->get_records_select('local_'.$tableprefix.'_metadata_field', $typestr, $params);

    foreach ($fields as $field) {
        require_once($CFG->dirroot.'/local/course_metadata/field/'.$field->datatype.'/field.class.php');
        ...
        if ($field->datatype == 'textarea' && !$formfield->is_locked()) {
            ...
            if ($formfield->data == $formfield->field->defaultdata) {
                ...
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'local_course_metadata', 'textarea', $formfield->fieldid);
            } else {
                $item = file_prepare_standard_editor($item, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'],
                                'local_course_metadata', $prefix, $formfield->dataid);
            }
        }
    }
}
...
function customfield_definition(&$mform, $item, $prefix, $typeid=0, $tableprefix) {
    ...
    $fields = $DB->get_records_select('local_'.$tableprefix.'_metadata_field', $typestr, $params, 'sortorder ASC');
    ...
    if ($display) {
        $mform->addElement('header', 'customfields', get_string('customfields', 'local_course_metadata'));
        foreach ($fields as $field) {
            require_once($CFG->dirroot.'/local/course_metadata/field/'.$field->datatype.'/field.class.php');
            $newfield = 'customfield_'.$field->datatype;
            $formfield = new $newfield($field->id, $item, $prefix, $tableprefix);
 ...
 * @param string $tableprefix Prefix to append '_metadata_field' to
...
function customfield_get_fields($item, $tableprefix, $prefix) {
    ...
    $fields = $DB->get_records('local_'.$tableprefix.'_metadata_field', array(), 'sortorder ASC');
...
function customfield_record($id, $tableprefix) {
    ...
    $fields = $DB->get_records('local_'.$tableprefix.'_metadata_field');
