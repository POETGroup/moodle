...
class customfield_textarea extends customfield_base {
    ...
    function edit_field_add(&$mform) {
        ...
        if ($this->is_locked()) {
            if ($this->data == $this->field->defaultdata) {
                $data = file_rewrite_pluginfile_urls($this->field->defaultdata, 'pluginfile.php', $context->id, 'local_course_metadata', 'textarea', $this->fieldid);
            } else {
                $data = file_rewrite_pluginfile_urls($this->data, 'pluginfile.php', $context->id, 'local_course_metadata', $this->prefix, $this->dataid);
            }
    ...
    */
    function edit_save_data($itemnew, $prefix, $tableprefix) {
        ...
        if ($dataid = $DB->get_field('local_'.$tableprefix.'_metadata', 'id', array($prefix.'id' => $itemnew->id, 'fieldid' => $data->fieldid))) {
            ...
            $data->id = $DB->insert_record('local_'.$tableprefix.'_metadata', $data);
        }
        $itemnew = file_postupdate_standard_editor($itemnew, $shortinputname, $TEXTAREA_OPTIONS, $TEXTAREA_OPTIONS['context'], 'local_course_metadata', $prefix, $data->id);
    ...
    * Display the data for this field
    ...
    static function display_item_data($data, $extradata=array()) {
        ...
        $context = context_system::instance();
        $data = file_rewrite_pluginfile_urls($data, 'pluginfile.php', $context->id, 'local_course_metadata', $extradata['prefix'], $extradata['itemid']);
