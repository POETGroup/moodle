...
class customfield_file extends customfield_base {
    ...
    function edit_load_item_data(&$item) {
        ...
        $this->data = file_prepare_standard_filemanager($item, $this->inputname, $FILEPICKER_OPTIONS, $FILEPICKER_OPTIONS['context'],
                                                           'local_course_metadata', $this->prefix . '_filemgr', $this->dataid);
    }
    ...
    function edit_save_data($itemnew, $prefix, $tableprefix) {
        ...
        $data->data = '';
        if ($dataid = $DB->get_field('local_'.$tableprefix.'_metadata', 'id', array('id' => $this->dataid, $prefix.'id' => $itemnew->id, 'fieldid' => $data->fieldid))) {
            $data->id = $dataid;
        ...
        $itemnew = file_postupdate_standard_filemanager($itemnew, $this->inputname, $FILEPICKER_OPTIONS, $FILEPICKER_OPTIONS['context'],
                                                                      'local_course_metadata', $this->prefix . '_filemgr', $data->id);
        $data->data = $data->id;
        $DB->update_record('local_'.$tableprefix.'_metadata', $data);
    ...
    */
    function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule($this->inputname.'_filemanager', get_string('customfieldrequired', 'local_course_metadata'), 'required', null, 'client');
        }
    }
    ...
    static function display_item_data($data, $extradata=array()) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'local_course_metadata', $extradata['prefix'] . '_filemgr', $data, null, false);
        if (count($files) < 1) {
            return get_string('nofileselected', 'local_course_metadata');
        } else {
