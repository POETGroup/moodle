...
class customfield_define_multiselect extends customfield_define_base {
    ...
    public function define_form_specific(&$form) {
        $title =  get_string('multiselectoptions', 'local_course_metadata');
        $this->define_add_js($form, '_specificsettings', false);
        $form->addElement('static', 'cautiondatele', '',
                get_string('multiselectdeleteunlink', 'local_course_metadata'));
        for ($menuind = 0; $menuind < self::MAX_CHOICES; $menuind++) {
            ...
            $group[] = $form->createElement('advcheckbox', 'default', '',
                    get_string('defaultselected', 'local_course_metadata'), array('class' => 'makedefault'));
        ...
        $form->addHelpButton('multiselectitem[0]', 'customfieldmultiselectoptions', 'local_course_metadata');
    }
        ...
        foreach ($data->multiselectitem as $ind => $item) {
            ...
            if ($item['option'] != '') {
                if (in_array($item['option'], $addoptions)) {
                    $err["multiselectitem[$ind]"] = get_string('menunotuniqueoptions', 'local_course_metadata');
                }
                $addoptions[] = $item['option'];
            }
        }
        if ($itemerr) {
            $err['multiselectitem[0]'] = get_string('menunooptions', 'local_course_metadata');
        }
        return $err;
    }
        ...
        if ($old) {
            ...
            foreach ($dataold->multiselectitem as $ind => $item) {
                if (!$data->multiselectitem[$ind]['delete'] &&
                    ...
                    $options = $DB->get_records('local_'.$tableprefix.'_metadata',
                    ...
                    foreach ($options as $option) {
                        ...
                        if (isset($itemdata[$oldhash])) {
                            ...
                            $DB->update_record('local_'.$tableprefix.'_metadata', $option);
                        }
                    }

                    // Rename: change param hashes.
                    $sql = 'UPDATE {'.'local_'.$tableprefix.'_info_data_param} SET value = ?
                            WHERE
                            value = ?
                            AND dataid IN
                                (SELECT id FROM {'.'local_'.$tableprefix.'_info_data} WHERE fieldid = ?)';
    ...
    public function define_add_js($form, $jsid = '', $limitone = true) {
        ...
        $jsmodule = array(
            ...
            'fullpath' => '/local/course_metadata/field/multiselect/multiselect.js',
            'requires' => array('json')
        );
        ...
        $iconjsmodule = array(
                        ...
                        'fullpath' => '/local/course_metadata/core/js/icon.picker.js',
                        'requires' => array('json'));
