...
class customfield_multiselect extends customfield_base {
    ...
    public function __construct($fieldid=0, $itemid=0, $prefix, $tableprefix) {
        ...
        if ($this->dataid) {
            // Load data settings.
            $params = $DB->get_records($this->tableprefix.'_metadata_param', array('dataid' => $this->dataid), '', 'value');
            $details = array();
            if ($params) {
                $details = $DB->get_field($this->tableprefix.'_metadata', 'data', array('id' => $this->dataid));
                if ($details != '') {
    ...
    public function edit_save_data_postprocess($key) {
        ...
        // Remove all params.
        if ($this->dataid) {
            $DB->delete_records($this->tableprefix . '_metadata_param', array('dataid' => $this->dataid));
        }
        foreach ($key as $ind => $value) {
            if ($value && isset($this->options[$ind])) {
                ...
                $DB->insert_record($this->tableprefix . '_metadata_param', $data);
            }
        }
    }
    ...
    public function edit_field_set_required(&$mform) {
        if ($this->is_required()) {
            $mform->addRule('grp_' . $this->fieldid . '_0',
                    get_string('customfieldrequired', 'local_course_metadata'), 'required', null,
                    'client');
        }
    }
