...
class customfield_checkbox extends customfield_base {
    ...
    function customfield_checkbox($fieldid=0, $item, $prefix, $tableprefix) {
        ...
        if (!empty($this->field)) {
            $datafield = $DB->get_field('local_'.$tableprefix.'_metadata', 'data', array($prefix.'id' => $item->id, 'fieldid' => $this->fieldid));
            if ($datafield !== false) {
                $this->data = $datafield;
            } else {
    ...
    function edit_field_add(&$form) {
        ...
        if ($this->is_required()) {
            $form->addRule($this->inputname, get_string('customfieldrequired', 'local_course_metadata'), 'nonzero', null, 'client');
        }
    }
